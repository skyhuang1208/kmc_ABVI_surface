...
double class_events::jump(){
	...
	double irates= cal_ratesI(isvcc, rates, ilist, nltcp, jatom); // WARNING: irates before vrates so the recb map can be generated
	double vrates= cal_ratesV(isvcc, rates, ilist, nltcp, jatom);
	...
	for(int i=0; i<rates.size(); i ++){
        if( (ran >= acc_rate) && (ran < (acc_rate + rates[i]/sum_rates) ) ){			
			if(isvcc[i])    actual_jumpV(ilist[i], nltcp[i], jatom[i]);
			else            actual_jumpI(ilist[i], nltcp[i], jatom[i]);
